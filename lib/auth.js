...
async function validateSession(token) {
  ...
  const result = await db.query(
    `SELECT s.*, u.org_id, u.role, u.email as user_email, u.name as user_name,
            ...
            m.profile_photo_url, m.bio, m.directory_visible, m.allow_messages
     FROM sessions s
  ...
  if (session.user_id) {
    ...
    return {
      type: 'member',
      sessionId: session.id,
      id: session.member_id,         // Routes use req.member.id
      memberId: session.member_id,   // Keep for backwards compatibility
      ...
      bio: session.bio,
      directoryVisible: session.directory_visible,
      allowMessages: session.allow_messages
    };
  }
