...
async function executeAction(action, contactId, orgId, client) {
  ...
  switch (type) {
    ...
    case 'send_email':
      return await sendEmailAction(contactId, config, client);

    case 'webhook':
      return await sendWebhook(contactId, config, client);
 ...
 */
async function sendEmailAction(contactId, config, client) {
  const { to, subject, body } = config;

  if (!subject || !body) {
    throw new Error('send_email requires subject and body');
  }

  // Get contact details for template variables
  ...
  const result = await sendEmail({ to: recipient, subject, body, contact });
  return result;
}
 ...
 */
async function sendWebhook(contactId, config, client) {
  ...
  const contactResult = await client.query(
    'SELECT id, first_name, last_name, email, company, phone, lead_score, source FROM crm_contacts WHERE id = $1',
    [contactId]
  );
  const contact = contactResult.rows[0];
